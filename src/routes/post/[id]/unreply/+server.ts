...
export const POST: RequestHandler = async ({ request, params, locals }) => {
	...
	if (!(await user_or_creator(params.id, session.user.email)))
		throw error(
			401,
			`You may not unreply with this post, for you do not own it, nor are you a contributor`
		);
	...
	if (
		...
		throw error(
			401,
			'You may not unreply to the specified post, for you do not own it, nor are you a contributor'
		);
